...
import { 
  ...
  Filter, Download, ChevronLeft, ChevronRight, Lock
} from "lucide-react";
...
export default function GestaoEsicPage() {
  ...
  const [statusFiltro, setStatusFiltro] = useState("PENDENTES"); // Padrão inteligente
  ...
  const carregarSolicitacoes = useCallback(async () => {
    try {
      ...
      setError("Falha ao carregar as solicitações do e-SIC. Verifique a conexão.");
    } finally {
      setLoading(false);
    }
  ...
  const exportarRelatorio = async (tipo: 'pdf' | 'csv') => {
    try {
      ...
      // Configuração para baixar arquivo
      ...
      setError(`Erro ao gerar ${tipo.toUpperCase()}. Tente novamente.`);
    }
  };
      ...
      <main className="flex-1 p-6 overflow-y-auto relative">
        <header className="flex items-center justify-between mb-8">
          ...
          <div className="flex gap-2">
            <button onClick={() => exportarRelatorio('csv')} className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-xl text-xs font-bold text-slate-700 hover:bg-slate-50 transition-all">
              <Download size={14} /> Exportar CSV
            </button>
            <button onClick={() => exportarRelatorio('pdf')} className="flex items-center gap-2 px-4 py-2 bg-slate-900 border border-slate-900 rounded-xl text-xs font-bold text-white hover:bg-slate-800 transition-all shadow-sm">
        ...
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 mb-6 flex flex-wrap gap-4 items-end animate-in slide-in-from-bottom-2">
          ...
          <div className="flex-1 min-w-[250px]">
            ...
            <div className="relative flex items-center">
              ...
              <input 
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-900 outline-none focus:ring-2 focus:ring-black focus:border-transparent transition-all"
              />
            </div>
          </div>
          ...
          <div className="w-[200px]">
            ...
            <select 
              ...
              className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-800 outline-none focus:ring-2 focus:ring-black transition-all cursor-pointer"
            >
          ...
          <div>
            ...
            <input 
              ...
              className="px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-900 outline-none focus:ring-2 focus:ring-black transition-all"
            />
          </div>
            ...
            <table className="w-full text-left text-sm whitespace-nowrap">
              ...
              <tbody className="divide-y divide-slate-100">
                {loading ? (
                  ...
                  <tr><td colSpan={6} className="px-6 py-12 text-center text-slate-400 font-bold uppercase tracking-wider text-xs">Nenhuma solicitação encontrada nos filtros atuais.</td></tr>
                ) : (
            ...
            <div className="p-4 border-t border-slate-100 bg-slate-50 flex items-center justify-between text-xs font-bold text-slate-500">
              ...
              <div className="flex items-center gap-4">
                <button 
                  ...
                  disabled={page === 0}
                  className="flex items-center gap-1 hover:text-black disabled:opacity-30 disabled:hover:text-slate-500 transition-colors"
                >
                  <ChevronLeft size={16} /> Anterior
                </button>
                <span className="bg-white px-3 py-1 rounded-md border border-slate-200 text-black">
            ...
            <div className="bg-[#F8FAFC] w-full max-w-3xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95">
              ...
              <div className="p-6 border-b border-slate-200 bg-white flex justify-between items-center">
                <div>
                  <h2 className="text-lg font-black text-slate-800 tracking-tight uppercase">
                    {isConcluido ? "Detalhes da Solicitação" : "Tramitar Solicitação"}
                  </h2>
                  <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-1">
                    Protocolo: {selectedRequest.protocolo}
                  </p>
                </div>
                <button onClick={fecharModal} className="text-slate-400 hover:text-black p-2 rounded-lg hover:bg-slate-100">
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="p-6 overflow-y-auto">
                {isConcluido && (
                  <div className="mb-6 p-4 bg-slate-100 border border-slate-200 rounded-xl flex items-center gap-3 text-slate-600">
                    ...
                    <div>
                      ...
                      <p className="text-xs mt-0.5">Esta solicitação já recebeu uma decisão final e está bloqueada para novas edições.</p>
                    </div>
                  </div>
                )}
                ...
                <div className="bg-white p-6 rounded-xl border border-slate-200 mb-6">
                  <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3 flex items-center gap-1"><FileText size={14}/> Mensagem Original</span>
                  ...
                  <form onSubmit={tramitarPedido} className="space-y-6 bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                    ...
                    <div>
                      ...
                      <select 
                        ...
                        className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl font-bold text-slate-800 outline-none focus:ring-2 focus:ring-black transition-all"
                      >
                    ...
                    <div>
                      ...
                      <textarea 
                        ...
                        className="w-full p-4 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-800 outline-none focus:ring-2 focus:ring-black resize-y transition-all"
                      />
                    </div>
                    <div className="flex justify-end pt-4 border-t border-slate-100 gap-3">
                      <button type="button" onClick={fecharModal} className="px-6 py-2.5 bg-white text-slate-700 border border-slate-200 rounded-xl font-bold text-xs uppercase hover:bg-slate-50 transition-all">
                        Cancelar
                      </button>
                      <button type="submit" disabled={processando} className="bg-black text-white font-bold py-2.5 px-8 rounded-xl shadow-lg hover:bg-slate-800 transition-all flex items-center gap-2 disabled:opacity-50 text-xs uppercase">
