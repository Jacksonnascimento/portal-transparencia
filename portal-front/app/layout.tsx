...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <html lang="pt-br" suppressHydrationWarning>
      ...
      <body className={`${inter.className} bg-slate-50 text-slate-900 flex flex-col min-h-screen relative`} suppressHydrationWarning>

        {/* LÓGICA DE CARREGAMENTO DO PORTAL */}
        {!isConfigLoaded ? (
          <div className="flex-grow flex items-center justify-center min-h-screen w-full bg-slate-50 absolute inset-0 z-10">
            <div className="animate-pulse text-slate-300 font-black uppercase tracking-widest text-[10px]">
              Preparando Portal...
            </div>
          </div>
        ) : (
          <div className="flex flex-col min-h-screen animate-in fade-in duration-500 w-full">
